...
class Attention(nn.Module):
    ...
    def forward(self, hidden_state, encoder_outputs):
        ...
        inputs = torch.cat((encoder_outputs, hidden_state), 2).reshape(-1, self.dim * 2)
...
class Encoder(nn.Module):
    def __init__(self, dim_vid, dim_hidden, input_dropout_p=0.2, rnn_dropout_p=0.5,
        ...
        self.dense = nn.Linear(dim_vid, dim_hidden)
        self.input_dropout = nn.Dropout(input_dropout_p)
        self.rnn_cell = nn.GRU
    ...
    def forward(self, vid_feats):
        batch_size, seq_len, dim_vid = vid_feats.shape
        vid_feats = self.dense(vid_feats.view(-1, dim_vid))
...
class GRUattModel(nn.Module):
    ...
    def forward(self, vid_feats, config=None, target_variable=None,
        ...
        seq_prob, seq_preds,all_seq_logprobs,all_seq_preds = self.decoder(encoder_outputs, encoder_hidden, config,
                                                                          target_variable, mode)

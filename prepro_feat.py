...
def extract_feats(params, model):
    ...
    train_val_frame_path = os.path.join(params['video_path'] + '/train/train/')
    train_val_frame_path_list = os.listdir(os.path.join(params['video_path'] + '/train/train'))
    #print("debug train_val_frame_path_list",train_val_frame_path_list)
    test_frame_path_list = os.listdir(os.path.join(params['video_path'] + '/test/test'))
    ...
    for frame_path in tqdm(test_frame_path_list, desc='processing test video data'):
        ...
        image_list = sorted(glob.glob(os.path.join(params['video_path'] + '/test/test/' + frame_path, '*.jpg')))
        samples = np.round(np.linspace(
            0, len(image_list) - 1, params['n_frame_steps']))
        image_list = [sample for sample in enumerate(samples)]
        print(image_list)
        images = torch.zeros((len(image_list), C, H, W))
        for iImg in range(len(image_list)):
            #img = load_image_fn(image_list[iImg])
            images[iImg] = image_list[iImg]#img
        with torch.no_grad():
            if params['gpu'] != '-1':
                images = images.cuda()
            fc_feats = model(images).squeeze()
...
if __name__ == '__main__':
    ...
    params = dict()
    params['output_dir'] = "videos"
    params['video_path'] = "../data_info"
